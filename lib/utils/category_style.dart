...
/// Maps an OpenTDB category name to a representative icon + background
/// color, since the API only returns id/name (no images).
class CategoryStyle {
  final IconData icon;
  final Color color;
  ...
  static CategoryStyle forName(String name, int fallbackIndex) {
    ...
    if (n.contains('general knowledge')) {
      return const CategoryStyle(Icons.public, Color(0xFFAFC9F0));
    }
    if (n.contains('book')) {
      return const CategoryStyle(Icons.menu_book, Color(0xFFBFE6C9));
    }
    if (n.contains('film') || n.contains('movie')) {
      return const CategoryStyle(Icons.movie, Color(0xFFF6C6CE));
    }
    if (n.contains('music')) {
      return const CategoryStyle(Icons.music_note, Color(0xFFDCC2F0));
    }
    if (n.contains('musical') || n.contains('theatre')) {
      return const CategoryStyle(Icons.theater_comedy, Color(0xFFF3E7B0));
    }
    if (n.contains('television') || n.contains('tv')) {
      return const CategoryStyle(Icons.tv, Color(0xFFAFC9F0));
    }
    ...
    if (n.contains('science: computers') || n.contains('computer')) {
      return const CategoryStyle(Icons.computer, Color(0xFFAFC9F0));
    }
    ...
    if (n.contains('science') || n.contains('nature')) {
      return const CategoryStyle(Icons.science, Color(0xFFDCC2F0));
    }
